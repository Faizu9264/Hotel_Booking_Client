  ...
  const filteredHotels = hotels.filter(
    (hotel) =>
      hotel.details.hotelName
        .toLowerCase()
        .includes(searchHotelName.toLowerCase()) &&
    ...
    <Card className="h-full w-full overflow-hidden" placeholder={"card"}>
      ...
      >
        <div className="mb-4 flex flex-col justify-between gap-8 md:flex-row md:items-center">
          ...
          <div className="flex w-full shrink-0 gap-2 md:w-max">
            <Button
              placeholder={"typography"}
              onClick={handleAddHotelsClick}
              className="flex items-center gap-3 text-black mt-3 mr-3"
              variant="outlined"
              size="sm"
            >
              Add Hotels
      ...
      <CardBody className="overflow-x-auto px-0" placeholder={"typography"}>
        <table className="w-full table-auto text-left">
          ...
          <tbody>
            {displayedHotels.map(
              ({ _id, details, location, images }, index) => {
                    ...
                    <td className={classes}>
                      <Tooltip content="Edit Hotel">
                        <IconButton
                          placeholder={"typography"}
                          variant="text"
                          onClick={() => handleEditHotelClick(_id)}
                        >
      ...
      >
        ...
        <div className="flex items-center gap-2">
          {[...Array(Math.ceil(hotels.length / pageSize)).keys()].map(
            (pageNumber) => (
              <Button
        ...
        <Button
          ...
          variant="outlined"
          size="sm"
          disabled={startIndex + pageSize >= hotels.length}
          onClick={() => handlePageClick(selectedPage + 1)}
        >
          Next
        </Button>
