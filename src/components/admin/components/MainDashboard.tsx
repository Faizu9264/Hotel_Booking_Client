...
import "chart.js";
import { UserData } from "./Users/userListing";
import { useDispatch, useSelector } from 'react-redux';
import { RootState } from '../../../redux/store';
import { Dispatch } from 'redux';
import adminApi from '../../../services/adminApi';
import { Booking } from "./Bookings/BookingListingTable";
// import { UserIcon, HotelIcon, HomeIcon, CalendarIcon } from "@heroicons/react/24/solid";
import { FontAwesomeIcon } from '@fortawesome/react-fontawesome';
import { faUsers, faHotel, faHome, faCalendar } from '@fortawesome/free-solid-svg-icons';
...
const MainDashboard = () => {
  ...
  const getRoomTypesData = (filteredRooms: Room[]): RoomType[] => {
    ...
    filteredRooms.forEach((room) => {
      ...
      roomTypesCount[roomType] = (roomTypesCount[roomType] || 0) + (roomsCount || 0);
    });
  
    const roomTypesData: RoomType[] = Object.entries(roomTypesCount).map(([name, count]) => ({
      name,
      count,
    }));
  ...
  bookings.forEach((booking) => {
    ...
    bookingsByMonth[monthYearKey] = (bookingsByMonth[monthYearKey] || 0) + booking.total;
  });
...
<div className="container mx-auto p-4" >
  ...
  <div className="count-card hover:shadow-lg transition-transform transform hover:scale-105 bg-blue-200 p-6 rounded-md text-center flex items-center">
    <FontAwesomeIcon icon={faUsers} className="h-8 w-8 text-blue-500 mr-2" />
  ...
  <div className="count-card hover:shadow-lg transition-transform transform hover:scale-105 bg-green-200 p-6 rounded-md text-center flex items-center">
    <FontAwesomeIcon icon={faHotel} className="h-8 w-8 text-green-500 mr-2" />
    <div className="flex flex-col items-center mx-4">
      <div className="text-xl font-bold mb-2">Hotels</div>
    </div>
    <div className="flex flex-col items-center">
      <div className="text-3xl font-bold text-green-800">{hotels.length}</div>
    </div>
  </div>
  ...
  <div className="count-card hover:shadow-lg transition-transform transform hover:scale-105 bg-yellow-200 p-6 rounded-md text-center flex items-center">
    <FontAwesomeIcon icon={faHome} className="h-8 w-8 text-yellow-500 mr-2" />
    <div className="flex flex-col items-center mx-4">
      <div className="text-xl font-bold mb-2">Rooms</div>
    </div>
    <div className="flex flex-col items-center">
      <div className="text-3xl font-bold text-yellow-800">{totalRoomsCount}</div>
    </div>
  </div>
  ...
  <div className="count-card hover:shadow-lg transition-transform transform hover:scale-105 bg-pink-200 p-6 rounded-md text-center flex items-center">
    <FontAwesomeIcon icon={faCalendar} className="h-8 w-8 text-pink-500 mr-2" />
    <div className="flex flex-col items-center mx-4">
      <div className="text-xl font-bold mb-2">Bookings</div>
    </div>
    <div className="flex flex-col items-center">
      <div className="text-3xl font-bold text-pink-800">{bookings.length}</div>
    </div>
  </div>
</div>
  ...
  <div className="flex flex-row"> 
    ...
    <div className="w-1/4 p-4">
      <h3 className="text-lg font-semibold mb-2 mb-0">Doughnut Chart</h3>
