...
import { setSingleRoomDetails } from "../../../../redux/slices/singleRoomSlice";
...
export const AddRooms = () => {
  ...
  useEffect(() => {
    if (isEditMode && roomId && rooms.length > 0) {
      ...
      if (roomToUpdate) {
        dispatch(setSingleRoomDetails({ selectedRoom: roomToUpdate }));
      }
    }
  ...
  const convertFieldsToNumbers = (roomDetails: RoomDetails) => {
    ...
    if (convertedDetails.rentAmount) {
      convertedDetails.rentAmount = parseFloat(
        convertedDetails.rentAmount.toString()
      );
    }

    if (convertedDetails.discountPrice) {
      convertedDetails.discountPrice = parseFloat(
        convertedDetails.discountPrice.toString()
      );
    }

    if (convertedDetails.roomsCount) {
      convertedDetails.roomsCount = parseInt(
        convertedDetails.roomsCount.toString(),
        10
      );
    }

    if (convertedDetails.maxPeople) {
      convertedDetails.maxPeople = parseInt(
        convertedDetails.maxPeople.toString(),
        10
      );
    }
