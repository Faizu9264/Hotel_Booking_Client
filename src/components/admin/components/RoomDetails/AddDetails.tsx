...
const AddDetails = () => {
  ...
  const singleRoomDetails =
    useSelector((state: RootState) => state.room.roomDetails.selectedRoom) ||
    {};
  ...
  const handleRentAmountChange = (e: ChangeEvent<HTMLInputElement>) => {
    ...
    if (value < minRent) {
      toast.error(
        `Rent amount must be greater than hotel minimum Rent ${minRent}`
      );
    } else if (value > 3500) {
  ...
  const handleHotelNameChange = (e: SelectChangeEvent<string | HotelData>) => {
    const selectedHotel: HotelData | undefined =
    ...
    >
      ...
      <CustomDialog open={viewHotelModal} onClose={handleCloseViewHotelModal}>
        ...
        >
          Hotel Details
          <IconButton
            onClick={handleCloseViewHotelModal}
            sx={{ marginLeft: "auto" }}
          >
            <CloseIcon />
          </IconButton>
      ...
      <Grid container spacing={2}>
        ...
        <Grid item xs={6}>
          <InfoField
            label="Rooms Count"
            type="number"
            value={singleRoomDetails.roomsCount || 1}
            disabled={true}
            fieldKey="roomsCount"
            minLength={5}
          />
