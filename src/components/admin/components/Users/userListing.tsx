...
export const UserListingTable: React.FC<UserListingTableProps> = () => {
  ...
  const displayedUsers: UserData[] = filteredUsers.slice(
    startIndex,
    startIndex + pageSize
  );
  ...
  const handleBlockUserClick = async (userId?: string): Promise<void> => {
    if (userId) {
      try {
        dispatch(adminApi.blockUser(userId));
        if (socket.current) {
    ...
    <Card className="h-full w-full overflow-hidden" placeholder={"card"}>
      ...
      <CardBody className="overflow-x-auto px-0" placeholder={"cardBody"}>
        <table className="w-full table-auto text-left">
          ...
          <tbody>
            {displayedUsers.map(
              (
                { _id, username, email, profileImage, phoneNumber, blocked },
                index
              ) => {
                ...
                return (
                  <tr key={_id}>
                    ...
                    <td className={classes}>
                      <Tooltip
                        content={blocked ? "Unblock User" : "Block User"}
                      >
                        <Button
                          variant="outlined"
                          onClick={() =>
                            blocked
                              ? handleUnblockUserClick(_id)
                              : handleBlockUserClick(_id)
                          }
                          size="sm"
      ...
      >
        ...
        <div className="flex items-center gap-2">
          {[...Array(Math.ceil(users.length / pageSize)).keys()].map(
            (pageNumber) => (
              <Button
                key={pageNumber + 1}
