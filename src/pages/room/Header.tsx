...
import { BookingState } from "../../redux/slices/bookingSlice";
  ...
  const bookingDetails = useSelector(selectBookingDetails);
    ...
    const handleSearch = () => {
      const updatedBookingDetails: Partial<BookingState & { roomDetails: { hotelId: string } }> = {
    ...
    console.log('updatedBookingDetails',updatedBookingDetails);
    
      dispatch(setBookingDetails(updatedBookingDetails));
      // navigate("/user/view-rooms", { state: { destination, date, options } });
    };
    
    
  
  
    const handleDateChange = (item: { selection?: { startDate: Date | undefined; endDate: Date | undefined } }) => {
